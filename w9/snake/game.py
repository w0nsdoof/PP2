...
import pygame , random , time , db , psycopg2
...
if db.is_username_exist(username):
    sql = """
    SELECT user_score, user_level FROM users_score
    WHERE user_name = %s
    """
...
def identify(username):
    db.connect()
    db.insert_username(username)
    db.insert_scores(username, score, lvl)
...
def main():
    ...
    while True:
        ...
        if len(snake.body) == 5 and len(snake.body) % 2 == 1:
            lvl += 1
            wall = Wall(lvl % 4)
